# Private Direct Chat - Supabase Setup

Run this exactly in Supabase SQL Editor on your project.

Preferred: run the SQL from `supabase/private_direct_chat_setup.sql` as a single script.

## 1) Schema update for direct private chat
...
## 2) Security (RLS) for private/direct visibility
...
Ensure `committee_messages` is in your publication (usually `supabase_realtime`).
...
This version is idempotent and safe to run repeatedly.
