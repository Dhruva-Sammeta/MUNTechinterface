...
export async function POST(req: Request) {
  try {
    ...
    let assignedDelegate: { id: string; user_id: string } | null = null;

    if (matched.assigned_user_id) {
      const assignedResult = await supabaseAdmin
        .from("delegates")
        .select("id,user_id")
        .eq("id", matched.assigned_user_id)
        .maybeSingle();
      if (assignedResult.error) {
        return NextResponse.json({ error: assignedResult.error.message }, { status: 500 });
      }
      const assigned = assignedResult.data as { id: string; user_id: string } | null;
      assignedDelegate = assigned;
      delegateId = assigned?.id ?? null;
    }

    if (existingDelegate?.id && delegateId && existingDelegate.id !== delegateId) {
      // Re-bind the passcode to the current authenticated delegate instead of forcing rotation.
      const { error: rebindErr } = await supabaseAdmin
        .from("delegate_passcodes")
        .update({ assigned_user_id: existingDelegate.id, assigned_at: new Date().toISOString() })
        .eq("id", matched.id);

      if (rebindErr) {
        return NextResponse.json({ error: rebindErr.message }, { status: 500 });
      }
    ...
    if (!delegateId && existingDelegate?.id) {
      delegateId = existingDelegate.id;
    }
    ...
    if (delegateId) {
      ...
      if (assignedDelegate?.id && assignedDelegate.user_id !== user.id) {
